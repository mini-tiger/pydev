...
def wheel():
	...
	# 找出两边的业务
	srcBizList, ok = returnBiz(url=srcUrlsuffix+'/api/c/compapi/v2/cc/search_business/')
	if ok != "sucess":
		logw.error(ok)
		os._exit(1)

	dstBizList, ok = returnBiz(url=dstUrlSuffix + '/api/v3/biz/search/0')
	# print dstBizList
	if ok != "sucess":
		logw.error(ok)
		os._exit(1)
	...
	for srcbiz in srcBizList:
		...
		srcHostList, ok = returnHostList(url=srcUrlsuffix + 'api/c/compapi/v2/cc/search_host/',
										 biz=srcbiz["bk_biz_name"])
		# for i in srcHostList:
		# 	print i['host']["bk_cpu"],i["host"]["bk_host_innerip"]
		...
		dstHostList, ok = returnHostList(url=dstUrlSuffix + '/api/v3/hosts/search', biz=srcbiz["bk_biz_name"])
		if ok != "sucess":
			logw.error(ok)
			os._exit(1)
