...
from sqlalchemy import create_engine, Column, Integer, String, DateTime, func, Float,Date
from sqlalchemy.ext.declarative import declarative_base
...
def delete_records_by_filename(filename):
    try:
        ...
        session.close()
def generate_instance_with_personnel(member, role, filename):
...
def generate_instance_with_project(filename, parsed_xml):
...
def insert_project(session, filename, parsed_xml):
    try:
        new_project = generate_instance_with_project(filename=filename, parsed_xml=parsed_xml)
...
def role_rule_replace(role):
    ...
    return role
def insert_personnel(session, filename, parsed_xml):
    try:
        for section, members in parsed_xml["root"].items():
            ...
            if isinstance(members["member"], list):
                for member in members["member"]:
                    new_personnel = generate_instance_with_personnel(member, role, filename)
