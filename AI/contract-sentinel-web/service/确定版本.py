...
def valid_file_version(file_path):
    elements = partition_docx(filename=file_path)
    return version_valid_by_elem(elements)

def version_valid_by_elem(elements):
    ...
    return record_cls,elements


# def word2txt(docpath, txtpath,source_type=False):
#     source_record_cls=None
#     if source_type:
#         source_record_cls,elem=valid_file_version(docpath)
#
#     elements = partition_docx(filename=docpath)
#     ele2txt(elements, txtpath,source_record_cls)
#
...
def convert_docx_to_txt(src_doc_dir, txt_dir):
    ...
    for filename in os.listdir(src_doc_dir):
        if filename.endswith(".docx"):
            ...
            word2txt(docpath=full_path, txtpath=new_full_path_txt,source_type=True)
            txt_dict.setdefault(full_path, new_full_path_txt)
            txt_list.append(new_full_path_txt)
    return txt_dict, txt_list


def list_filter(q1, q2, file_list, un=False):
    tlist = [file for file in file_list if q1 in file and q2 in file]

    if un:
        return [file for file in tlist if file.find("un") != -1]
    else:
        return [file for file in tlist if file.find("un") == -1]
...
#         return list_filter(q1="dyx", q2="reserved", file_list=file_list, un=False)
#
#     if dyx and reserved == False:
#         return list_filter(q1="dyx", q2="unreserved", file_list=file_list, un=True)
#
#     if dyx == False and reserved:
#         return list_filter(q1="sjhl", q2="reserved", file_list=file_list, un=False)
#
#     if dyx == False and reserved == False:
#         return list_filter(q1="sjhl", q2="unreserved", file_list=file_list, un=True)
