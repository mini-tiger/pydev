...
import { computed, onMounted, onUnmounted, onUpdated, ref, inject } from 'vue'
...
const viewLogTracing = inject('viewLogTracing')
const callViewLogTracing = (chatUuid: any) => {
	viewLogTracing(chatUuid)
}
</script>
...
<template>
  <div class="text-black" :class="wrapClass">
    <div ref="textRef" class="leading-relaxed break-words">
      <div v-if="!inversion" class="flex flex-row">
        <div v-if="!asRawText" class="markdown-body" v-html="text" />
        <div v-else class="whitespace-pre-wrap" v-text="text" />
				<div v-if="text.includes('正在生成报告中')" style="display: flex; align-items: flex-end; margin-bottom: 5px;">
					<LoadingDots />
				</div>
      </div>
      <div v-else class="whitespace-pre-wrap" v-text="text" />
	...
	<div v-if="!inversion && !system" :class="wrapClass" style="background-color: transparent; padding-left: 4px;">
		<NButton class="titleBtn" text style="color:#2853E0; font-size: 12px;" @click="callViewLogTracing(chatUuid)">查看日志</NButton>
	</div>
</template>
