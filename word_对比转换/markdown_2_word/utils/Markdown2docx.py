...
from word_对比转换.markdown_2_word.utils.PreprocessMarkdown2docx import PreprocessMarkdown2docx
'''
    For docx - see
...
def add_custom_heading(doc, text, font_size, alignment, level):
    ...
    for line in soup:
        if line is not None:
            ...
            if line.name == 'h1':
                ...
                # 添加一个自定义标题，并在其后添加分页符
                add_custom_heading(doc, line, 26, WD_ALIGN_PARAGRAPH.CENTER, 1)
                continue
            if line.name == 'h2':
